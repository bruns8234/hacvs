	...
	this.draw = function (colorKey, hitContext) {
		...
		if (this.configValid) {
			...
			if (colorKey !== undefined) {
				...
				// Eine BAR in der Farbe gemäß config.color:elementMode ausgeben
				var eColor = getColorValue(config.color, elementMode);
				context.save();
				context.translate(x, y);
				context.fillStyle   = eColor;
				context.fillRect(drawSet.x, drawSet.y, drawSet.width, drawSet.height);
				context.restore();
			}
...
/**
	@description    Element: CAP
					Dieses Element dient zum Abschluss eines BAR- oder TEXT-Elements. Sie dienen hauptsächlich zur Ein-
					fassung von BUTTON-Elementen, können aber auch z.B. am Ende einer BAR platziert werden.
	@param			{Number}			pRaster.gridWidth				Breite einer Rasterspalte in Pixel
	@param			{Number}			pRaster.gridHeight				Höhe einer Rasterzeile in Pixel
	@param			{Number}			pRaster.maxWidth				Anzahl der verfügbaren Rasterspalten
	@param			{Number}			pRaster.maxHeight				Anzahl der verfügbaren Rasterzeilen
	...
	this.draw = function (colorKey, hitContext) {
		...
		if (this.configValid) {
			...
			if (colorKey !== undefined) {
				...
				// Nun einen Hintergrund mit der im colorset definierten Hintergrundfarbe zeichnen
			...
			switch (config.scaling.toLowerCase()) {
				...
				case 'cut':    // Bild passend zuschneiden, so das es zentriert im Anzeigebereich liegt
					...
					if ((width >= imageWidth) && (height >= imageHeight)) {
						...
						sWidth  = imageWidth;
						sHeight = imageHeight;
						dX      = Math.max(0, Math.floor((pixLoc.width - imageWidth) / 2 + 0.9));
						dY      = Math.max(0, Math.floor((pixLoc.height - imageHeight) / 2 + 0.9));
						dWidth  = imageWidth;
						dHeight = imageHeight;
					} else {
						// Hier müssen wir cutten, das Bild ist größer als der Anzeigebereich...
						sX       = Math.max(0, Math.floor((imageWidth - pixLoc.width) / 2 + 0.9));
						sY       = Math.max(0, Math.floor((imageHeight - pixLoc.height) / 2 + 0.9));
						sWidth   = Math.min(imageWidth, pixLoc.width);
						sHeight  = Math.min(imageHeight, pixLoc.height);
						dX       = Math.max(0, Math.floor((pixLoc.width - imageWidth) / 2 + 0.9));
						dY       = Math.max(0, Math.floor((pixLoc.height - imageHeight) / 2 + 0.9));
						dWidth   = width;
						dHeight  = height;
...
/**
	...
	@param			{object}			pContext						Ref. auf einen Canvas-2D-Context zur Darstellung des Elements
	@param			{object}			pSoundManager					Ref. auf die SoundManager-Instanz, die zur Soundausgabe verwendet wird.
**/
function ElementBUTTON(pRaster, pConfig, pContext, pSoundManager, pDataManager) {
	...
	this.click = function (myID, mainObject) {
		...
		if (this.configValid) {
			...
			if (!config.condition) {
				...
				console.log('Bedingungsprüfung:  Vergleiche [' + config.conditionID + '] (' + mainObject.getTerminalStorage(config.conditionID) + ') mit "' + config.conditionValue + '"');
 ...
 */
 ...
 /**
	...
	@param			{object}			pContext						Ref. auf einen Canvas-2D-Context zur Darstellung des Elements
**/
function ElementIMGVALUE(pRaster, pConfig, pContext, pPictureManager, pDataManager) {
	...
	var config          = null;                 // Elementkonfiguration
	var context         = null;                 // Ref. auf 2D-Context für die Darstellung
	var pictureManager  = null;					// Ref. auf die zu verwendene PictureManager-Instanz
	var dataManager     = null;					// Ref. auf die zu verwendene DataManager-Instanz
	var elementMode     = '';                   // Aktueller Zustand des Elements
	var lastMode        = '';                   // Der letzte Zustand des Elements
	...
	**/
	...
	@param			{function}	addActionFunction	Funktion aus HACVS mit der ein neuer Eintrag in die Action-Queue geschrieben werden kann
	**/
	this.valueUpdate = function (newValue, addActionFunction) {
		
		// Nur wenn wirklich der Wert geändert ist müssen wir etwas tun...
		if (newValue !== variableValue) {
			
			// Gemäß variableType die entsprechende Auswahlprüfung durchlaufen
			switch (variableType) {
			case 0:
				imageObject.setPicture(referenceList[newValue].image);		// Das entsprechende Bild im PICTURE-Element setzen
				mainObject.addAction(0, [ { id: myID, action: 'setMode', parameter: ['highlight'] },
										  { id: myID, action: 'update',  parameter: [] } ]);
	...
	if (!result.valid) {
		...
		var pairList = [];
		var refList  = [];        // Enthält eine Liste aller Resourcen mit den Vergleichswerten
		var defPair  = {key: '~DEFAULT', resource: 'NO_IMAGE'};
		...
		for (var key in pairList) {
			...
			// Wenn das das Default-Pair ist, dann in defPair speichern,
			if (typeof pair[0] === 'string' && pair[0].toUpperCase() === '~DEFAULT') {
				defaultImage = pair[1];
			} else {
				// sonst den Eintrag an refList anhängen.
				refList.push({key: pair[0], image: pair[1]});
			}
		}

		// Die PICTURE-Element erzeugen und die Ref. speichern
		...
		dataManager.registerVariable(config.varID, this.valueUpdate);
...
/**
	...
	@param			{object}			pContext						Ref. auf einen Canvas-2D-Context zur Darstellung des Elements
	@param			{object}			pSoundManager					Ref. auf den zu verwendenen SoundManager
	@param			{object}			pPictureManager					Ref. auf den zu verwendenen PictureManager
**/
function ElementSOUNDTOGGLE(pRaster, pConfig, pContext, pSoundManager, pPictureManager) {
	...
	var config          = null;					// Elementkonfiguration
	var context         = null;					// Ref. auf 2D-Context für die Darstellung
	var soundManager    = null;					// Ref. auf den SoundManager
	var pictureManager  = null;					// Ref. auf den PictureManager
	var drawSet         = null;					// Zeichekoordinaten (berechnet der Konstruktor)
	var elementMode     = '';					// Aktueller Zustand des Elements
	var lastMode        = '';					// der letzte Zustand des Elements
	...
	this.draw = function (colorKey, hitContext) {
		...
		if (this.configValid) {
			...
			if (colorKey !== undefined) {
				...
				// Als erstes eine Fläche in der Modus-Farbe zeichnen
				var eColor = getColorValue(config.color, elementMode);
				context.save();
				context.translate(x, y);
	...
	**/
	this.click = function (myID, mainObject) {
		...
		if (this.configValid) {
			...
			var status = soundManager.toggleSound();
			...
			imageObject = pictureManager.getPicture(drawSet.name);
	...
	if (!result.valid) {
		...
		// Sound- und PictureManager-Instanzen ins Objekt übernehmen
		soundManager   = pSoundManager;
		pictureManager = pPictureManager;
		...
		var name = soundManager.getEnabled() ? config.pictureOn : config.pictureOff;
